...
export function TaskSearch({ priority, query, status }: TaskSearchProps) {
  return (
    <form
      action="/tarefas"
      method="get"
      className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_190px_180px_auto]"
    >
      ...
      <label className="flex min-h-12 items-center rounded-xl border border-slate-200 px-4 focus-within:border-brand-500 focus-within:ring-4 focus-within:ring-brand-100">
