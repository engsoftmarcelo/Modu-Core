...
export function Logo({ compact = false, className, href = "/inicio" }: LogoProps) {
  return (
    ...
    >
      <span className="relative grid size-11 shrink-0 place-items-center overflow-hidden rounded-xl bg-ink-950 shadow-lg shadow-indigo-950/15">
      ...
      {!compact && (
        <span className="text-xl text-ink-950">ModuCore</span>
      )}
    </Link>
  );
}
